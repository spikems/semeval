...
def connect(dsn='localhost:next',user='',password=''):
    if not dsn:
...
def db_read_data(handler, table_name, expression, *column):
    ...
    if expression:
        ...
        if rst:
            ...
            _column = data['column'].strip()
            d1 = string.atoi(data['d1'].strip())
            d2 = string.atoi(data['d2'].strip())
            condition = {_column:{op1:d1,op2:d2}}
        else:
            ...
            if rst:
                ...
                if op1_str == "=":
                    condition = {_column:d1}
                else:
                    op1 = op_map2[data['op'].strip()]
                    condition = {_column:{op1:d1}}
            else:
