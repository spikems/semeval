...
from find_words.generate_suffix_dict import generate_right_suffix_dict,generate_left_suffix_dict
...
class WordsFinder(object):
    ...
    def find_words(self):
        ...
        """
        # start = time.time()
        self.spans = text_preprocess(text_list=self.text_list, seg_label="...")
        print self.spans
        # end = time.time()
        ...
        self.result = find_unknown_word(unknown_words=self.unknown_words,freq_IS_IG_product_cut=self.freq_IS_IG_product_cut,
...
if __name__ == '__main__':
    ...
    ab = WordsFinder(text_list=text_list)
    ab.find_words()

    for res in ab.result:
        print res
