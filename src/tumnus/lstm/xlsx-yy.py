...
def checksheet(xl_workbook, sheet_id):
    ...
    for idx, cell_obj in enumerate(row):
        ...
        colname = cell_obj.value.strip()
        if colname in idmap:
            idmap[colname] = idx
    
            
    logger.info('idmapping: %s', ' '.join(['%s:%s'%(x.encode('utf-8'), idmap[x]) for x in idmap]))
    return idmap
...
def dumpsheetEx(xl_workbook, sheet_id, idmap, outfname):
    ...
    print('before write nullcnt = %d'%nullcnt)
    ...
    for arow in sheet:
        try:
            #try to verify it's an id, otherwise skip this record
            _id = float(arow[6].encode('utf-8'))
            ...
            #output by id
            #if nullcnt < 100:
            #    logger.info('%s %s %s %s\n'%(arow[0], arow[6], arow[2], arow[3]))
            outf.write('%s %s %s %s\n'%(arow[0], arow[6].encode('utf-8'), trim(arow[2].encode('utf-8')), trim(arow[3].encode('utf-8'))))

            # database
            datf.write('%s %s %s %s\n'%(arow[6].encode('utf-8'), arow[5].encode('utf-8'), arow[7].encode('utf-8'), arow[4].encode('utf-8')))
