...
def get_classifier(classifier = 'lr', balanced = True):
    ...
    if classifier == "lsvc":
        ...
        clf = RandomForestClassifier(n_estimators=400, n_jobs=-1,criterion='entropy', class_weight = class_weight)
    elif classifier == 'lr':
        ...
        #clf = xgb.XGBClassifier(booster = 'dart', learning_rate=0.05, subsample = 0.8, n_estimators = 400, max_depth = 8, min_child_weight = 1, scale_pos_weight = 0.018767 if balanced else 1)
        clf = xgb.XGBClassifier(booster = 'dart', nthread = -1, subsample = 0.8, n_estimators = 400, max_depth = 8, min_child_weight = 1, scale_pos_weight = 0.018767 if balanced else 1)
    else:
        clf = None
