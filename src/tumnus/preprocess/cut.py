...
Usage:
    cut -t [pos] -c [hidden]  -f <file name> --template <template path> --brands <brand path or single brand name>
        ...
        --userdict ; set user dictionary when load jieba
"""
...
if __name__=="__main__":
    ...
    if opt.userdict:
        if os.path.exists(opt.userdict):
            logger.info('jieba load opt.user dictionary: %s', opt.userdict)
            jieba.load_userdict(opt.userdict)
        else:
            logger.info('jieba opt.user dictionary %s not found!', opt.userdict)
