    ...
    def dumpsheet(self, sheet_id, outfname, dataset, use_idcol = True):
        ...
        for row_idx in range(0, xl_sheet.nrows):    # Iterate through rows
            ...
            for getid in self._dumpcol[dataset]:
                cell_obj = xl_sheet.cell_value(row_idx, getid)  # type
                if type(cell_obj) not in [unicode, str]:
                    arow.append(str(int(cell_obj)))
                else:
                    arow.append(cell_obj)
            #check the record
            if arow[0] not in self._labels[dataset]:
                nullcnt += 1
                continue
            else:
        ...
        delcnt = 0
        if not use_idcol:
            sheet = sorted(sheet, key = lambda arow: arow[4])
        for arow in sheet:
            label = self._labels[dataset][arow[0]]
            #patch here 
            ...
            try:
                ...
                if use_idcol:
                    txtf.write('%s %s %s %s\n'%(label, int(arow[3]) , trim(arow[1].lower().encode('utf-8')), trim(arow[2].lower().encode('utf-8'))))
                else:
                    txtf.write('%s %s%s %s %s\n'%(label, int(arow[3]), arow[4], trim(arow[1].lower().encode('utf-8')), trim(arow[2].lower().encode('utf-8'))))
            except:
                #known exceptions,  
